...
export const DiseaseSelectionInput = ({ diseaseSelectionValue, setDiseaseSelection, diseasesList, setDiseaseList }) => {
    ...
    const [spanIsVisible, setSpanIsVisible] = useState(
        !!searchParams.get("diseaseSelection")
    );

    /* HANDLE DISEASES LIST SELECTED FROM MULTI-SELECTOR */
    /* const [diseasesList, setDiseaseList] = useState([]); */


    /* HANDLE DROPDOWN WRAPPER WHEN PAGE IS RELOADED */
    useEffect(() => {
        if (searchParams.get('diseaseSelection')) setDropdownIsVisible(false);
    ...
    const handleValueSelection = (selectedValue) => {
        /* selectedValue = value on input, that comes from autocomplete wrapper */
        
        /* crea la lista de datos seleccionada ya filtrada */
        setDiseaseList(disease => [...new Set([...disease, selectedValue])]);
        
        setDropdownIsVisible(false);

        /* no se usa por ahora.... no queda nada seleccionado */
    ...
    const onClickSpan = (e) => {
        ...
        /* TODO: APLICAR ESTO AL CONTAINER, PARA QUE ELIMINE EL PARAMS DEL BOTÓN DESELECCIONADO */
            ...
            >
                <div className="relative w-full flex flex-col items-center" ref={containerRef}>
                    <input
                        ...
                        type="text"
                        placeholder="Arritmia, Acné, Salud Mental..."
                        value={diseaseSelectionValue}
